...
export function buildReminderEmail(
  ...
  const subject = isOverdueKind
    ? `⚠️ Urgent: Invoice #${input.invoiceId} (${amount}) is OVERDUE`
  ...
  const opening = isOverdueKind
    ...
    : `This is a friendly reminder that invoice #${input.invoiceId} for ${amount} is due on ${due}.`;

  const closing = isOverdueKind
    ? "Please arrange for payment as soon as possible. If payment has already been sent, please disregard this notice."
    ...
    <html>
      <head>
        ...
        <style>
          ...
          .detail-row { display: flex; justify-content: space-between; font-size: 13px; color: #94a3b8; margin-top: 6px; }
      ...
      <body>
        <div class="container">
          ...
          <div class="footer">
            Sent via InvoiceTracker — Simple, High-Speed Invoice Management
          </div>
        </div>
      </body>
    </html>
